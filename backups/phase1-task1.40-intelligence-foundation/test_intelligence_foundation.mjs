...
async function run() {
  ...
  const EXPECTED_FILES = ['PHASE4_CAPABILITY_CONSOLIDATION_REVIEW.md', 'PHASE4_CAPABILITY_PLAN.md', 'PHASE4_CONSOLIDATION_REVIEW.md', 'PHASE4_DECISION_CAPABILITY_REVIEW.md', 'PHASE4_DECISION_CONTRACT_PLAN.md', 'PHASE4_DECISION_FLOW_PLAN.md', 'PHASE4_DECISION_INTEGRATION_PLAN.md', 'PHASE4_DECISION_ORCHESTRATION_INTEGRATION.md', 'PHASE4_DECISION_OUTPUT_EVOLUTION.md', 'PHASE4_FINAL_VALIDATION_AND_PHASE5_PLAN.md', 'PHASE5_PRODUCT_INTEGRATION_PLAN.md', 'README.md', 'analysis_engine.js', 'contracts.js', 'index.js', 'insight_service.js', 'recommendation_engine.js'];
  // TASK1.90後更新：Phase 5開始，新增了
  // PHASE5_PRODUCT_INTEGRATION_PLAN.md（Phase 5 Product
  // Integration Architecture Planning文件），同樣是明確的文件
  // 補充，不是回歸，加入預期清單。
  // TASK1.89後更新：新增了
  // PHASE4_FINAL_VALIDATION_AND_PHASE5_PLAN.md（Phase 4 Final
  // Validation & Phase 5 Transition Planning文件），同樣是明確的
  // 文件補充，不是回歸，加入預期清單。
  // TASK1.88後更新：新增了PHASE4_CAPABILITY_CONSOLIDATION_REVIEW.md
  // （Phase 4 Intelligence Capability Consolidation Review文件），
  ...
  // TASK1.75後更新：新增了PHASE4_CAPABILITY_PLAN.md（Phase 4
  // Intelligence Capability Architecture Planning文件），同樣是
  // 明確的文件補充，不是回歸，加入預期清單。
  ...
  // TASK1.82後更新：新增了PHASE4_DECISION_CAPABILITY_REVIEW.md
  // （Decision Capability Boundary Architecture Review文件），
  // 同樣是明確的文件補充，不是回歸，加入預期清單。
  // TASK1.84後更新：新增了PHASE4_DECISION_CONTRACT_PLAN.md
  // （Decision Capability Contract & Output Architecture Plan
  // 文件），同樣是明確的文件補充，不是回歸，加入預期清單。
  // TASK1.85後更新：新增了PHASE4_DECISION_INTEGRATION_PLAN.md
  // （Decision Capability Integration Architecture文件），同樣是
  // 明確的文件補充，不是回歸，加入預期清單。
  ...
  await test(`（TASK1.90後更新）src/intelligence/ 底下的檔案（不含子目錄）恰好是TASK1.40規格要求的6個加上TASK1.75/1.80/1.81/1.82/1.84/1.85/1.86/1.87/1.88/1.89新增的10份Phase 4規劃/審查文件，加上TASK1.90新增的1份Phase 5規劃文件`, () => {
  ...
  await test('（TASK1.69後更新）app.intelligence 恰好具備  二十四個欄位', () => {
    const app = createApplication(makeFullEnv());
    assert.deepStrictEqual(Object.keys(app.intelligence).sort(), ['analysis', 'analysisEngine', 'application', 'behaviorFeature', 'capabilities', 'context', 'dataPreparation', 'events', 'execution', 'facade', 'features', 'governance', 'history', 'insightExecutionFlow', 'insightFeature', 'insightService', 'metrics', 'monitoring', 'orchestration', 'recommendation', 'recommendationEngine', 'service', 'useCases', 'workflow']);
  });
