...
async function run() {
  ...
  // TASK1.37新增GET/PATCH /api/profile同樣掛了這兩個middleware；TASK1.38
  // 新增GET /api/timeline同樣掛了這兩個middleware——這裡的排除清單同步
  // 更新。
  await test('（TASK1.38後更新）原始碼掃描：router.js 支援每條路由各自middlewares，除了auth六條已啟用路由跟十條User Data API跟dashboard跟profile跟timeline外其餘皆是空清單', () => {
    const router = createAppRouter();
    ...
    const activatedPaths = [
      ...
      '/api/explorations', '/api/food-events', '/api/emotions', '/api/behaviors', '/api/reports', '/api/dashboard', '/api/profile', '/api/timeline',
    ];
