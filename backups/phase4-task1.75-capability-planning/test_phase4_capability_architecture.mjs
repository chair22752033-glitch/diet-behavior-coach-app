...
async function run() {
  ...
  // （TASK1.79後更新）原本這裡有一個「Application Layer（application/
  // 整個目錄樹）本次任務完全沒有被修改」的斷言，比對即時的git diff
  // --stat。這是跟TASK1.39/1.56/1.63/1.67/1.68/1.76同一種「比對
  // 即時git diff」的脆弱治具：application/整個目錄樹從來就不在本
  // 任務系列真正的禁止清單裡，TASK1.79合法地在
  // `application/features/`底下新增了第三個Intelligence
  // Application Feature domain（"intelligence"）——這不是TASK1.75
  ...
  // 完全沒有被TASK1.75修改」的斷言，比對整個檔案即時的git diff
  ...
  // 導致這個斷言失敗——這不是TASK1.75造成的回歸，而是斷言本身
