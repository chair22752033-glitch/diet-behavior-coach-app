...
async function run() {
  ...
  await test('createAppRouter()（不傳參數）行為與TASK1.21～1.25完全一致，不含legacy路由', () => {
    const router = createAppRouter();
    const paths = router.routes.map((r) => r.path);
    assert.ok(!paths.includes('/'));
    assert.ok(!paths.includes('/api/sync'));
    assert.strictEqual(router.routes.length, 5); // 4個auth + 1個user
  });
  ...
  await test('（延續auth/user route）createAppRouter(legacyHandler) 同時具備auth/user/legacy三種路由，總數為14', () => {
    const router = createAppRouter(makeLegacyHandler());
    assert.strictEqual(router.routes.length, 14);
  });
