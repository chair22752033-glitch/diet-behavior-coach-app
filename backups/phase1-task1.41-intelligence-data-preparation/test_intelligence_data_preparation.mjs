...
async function run() {
  ...
  // 回歸，這裡的預期key清單已同步更新。
  // 注意：TASK1.48 又新增了 `facade` 欄位，這是明確要做的擴充，不是
  // 回歸，這裡的預期key清單已同步更新。
  await test('（TASK1.48後更新）app.intelligence 仍然保留TASK1.40既有的insightService/analysisEngine/recommendationEngine跟TASK1.41既有的dataPreparation跟TASK1.42既有的context跟TASK1.43既有的analysis跟TASK1.44既有的recommendation跟TASK1.45既有的orchestration跟TASK1.46既有的service（沒有被TASK1.48取代或破壞）', () => {
    const app = createApplication(makeFullEnv());
