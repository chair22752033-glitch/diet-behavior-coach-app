...
async function run() {
  ...
  await test('（5.runtime isolation）src/bootstrap/application.js的intelligence物件維持22個欄位不變（本次任務沒有新增bootstrap欄位）', async () => {
  ...
  await test('（11.P1-P6）src/bootstrap/application.js 完全沒有被TASK1.68修改（本次任務不需要在bootstrap組裝這個純函式工具）', () => {
