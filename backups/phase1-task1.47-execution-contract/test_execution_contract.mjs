...
async function run() {
  ...
  // 這裡的預期key清單已同步更新。
  await test('（TASK1.55後更新）app.intelligence 恰好具備TASK1.46既有九個欄位加上TASK1.48新增的facade、TASK1.50新增的execution、TASK1.51新增的events、TASK1.52新增的history、TASK1.53新增的monitoring、TASK1.54新增的metrics、TASK1.55新增的governance，共十六個欄位', () => {
    const app = createApplication(makeFullEnv());
