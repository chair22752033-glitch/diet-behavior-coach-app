...
async function run() {
  ...
  // 這裡的預期key清單已同步更新。
  await test('（TASK1.54後更新）app.intelligence 恰好具備十五個欄位（TASK1.48既有十個加上TASK1.50新增的execution、TASK1.51新增的events、TASK1.52新增的history、TASK1.53新增的monitoring、TASK1.54新增的metrics）', () => {
    const app = createApplication(makeFullEnv());
